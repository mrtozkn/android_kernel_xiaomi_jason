...
prompt_build_kernel() {
    ...
    # if [ "$compile_kernel" == "y" ]; then
        ...
        if (( $(echo "$host_glibc <= 2.35" | bc -l) )) || [ "$toolchain" == "neutron" ]; then
            ...
            make -j$(nproc) ARCH=arm64 PATH=$PATH:/home/system/toolchains/neutron-clang/bin  CC=/home/system/toolchains/neutron-clang/bin/clang CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi- LLVM=1 LLVM_IAS=1 O=out
        else
